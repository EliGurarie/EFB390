...
subtitle: '[EFB 390](https://eligurarie.github.io/EFB390/) - Fall 2025'
...
The difference between these two is <u>confusing</u> and <u>important</u>.  Here are some bullet points: 

- The [standard deviation (SD)](https://en.wikipedia.org/wiki/Standard_deviation) is a measure of total spread of data.  

- The [standard error (SE)](https://en.wikipedia.org/wiki/Standard_error) is a measure of the precision of an estimate.  You need it for constructing **confidence intervals**. 

- The **standard deviation** is (basically) always estimates with the same formula: 
$$\text{SD}(N) = \sqrt{\frac{1}{k-1}\sum_{i= 1}^k \left(N_i - \widehat{N} \right)^2}$$
...
2. Next, square them and sum them:
...
3. Divide that thing by $k-1$ and take the square root of the whole thing:
...
Notice also that this time we saved the output into an object called `count.sd`. 
